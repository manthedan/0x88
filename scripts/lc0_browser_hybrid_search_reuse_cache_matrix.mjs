...
function parseArgs(argv) {
  ...
  for (const [name, value] of [['port', args.port], ['layers', args.layers], ['eval-iters', args.evalIters], ['eval-warmup', args.evalWarmup], ['search-iters', args.searchIters], ['search-warmup', args.searchWarmup], ['timeout', args.timeoutMs]]) {
...
async function runCell(args, combo, index, total) {
  ...
  process.stderr.write(`[reuse-cache] ${index}/${total} backend=${combo.headBackend} visits=${combo.visits} batch=${combo.batch} reuse=${combo.reuseTree ? 1 : 0} cache=${combo.evalCacheEntries}\n`);
