#!/usr/bin/env node
// Node bench/parity harness for Berserk Emscripten artifacts: drives the
...
//
// Usage:
//   node scripts/berserk_emscripten_bench.mjs \
//     --js public/berserk/berserk-emscripten.js,public/berserk/berserk-emscripten-simd128.js \
...
async function loadModule(jsPath) {
  ...
  for (const ext of ['js', 'wasm', 'data']) {
    ...
    if (!fs.existsSync(file)) throw new Error(`Missing Berserk artifact: ${file}`);
  }
  // Copy to .cjs so Node's createRequire treats the modularized output as CJS.
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'berserk-bench-'));
