...
const jsPath = path.resolve(args.get('js') ?? path.join(root, 'public', 'stormphrax', 'stormphrax-emscripten.js'));
const depth = Math.max(1, Math.floor(Number(args.get('depth') ?? 1)));
const timeoutMs = Math.max(1000, Math.floor(Number(args.get('timeout') ?? 30000)));
...
const sharedDataPath = path.join(path.dirname(jsPath), 'stormphrax-emscripten.data');
const dataPath = fs.existsSync(variantDataPath) ? variantDataPath : sharedDataPath;

for (const file of [jsPath, wasmPath, dataPath]) {
  if (!fs.existsSync(file)) throw new Error(`Missing Stormphrax Emscripten artifact: ${file}`);
}

const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'stormphrax-emscripten-smoke-'));
...
const Stormphrax = require(tmpJs);
const stdout = [];
const stderr = [];
...
if (!module._isReady?.()) throw new Error('Stormphrax module did not report ready');
...
const expectedStart = args.get('expect-start');
const expectedFen = args.get('expect-fen');
if (expectedStart && startBestmove !== `bestmove ${expectedStart}`) throw new Error(`Expected startpos bestmove ${expectedStart}, got ${startBestmove}`);
if (expectedFen && fenBestmove !== `bestmove ${expectedFen}`) throw new Error(`Expected FEN bestmove ${expectedFen}, got ${fenBestmove}`);
const report = {
  jsPath,
  wasmPath,
  dataPath,
