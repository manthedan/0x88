...
function parseArgs(argv) {
  const args = parseScriptArgs(argv, {
    options: {
      ...
      'skip-arena': { type: 'boolean', default: false },
...
async function runAnalysisSmoke(args) {
  ...
  try {
    ...
    const status = await pollUntil(
      ...
      (value) => validateStrictCustomAudit('analysis', value) && String(value.lines ?? '').includes('Centipawn · custom WebGPU'),
      args.timeoutMs,
    );
    return {
      name: 'analysis',
      status: 'LC0_CENTIPAWN_ANALYSIS_STRICT_CUSTOM_WEBGPU_DONE',
      url: String(url),
      auditEvents: strictCustomAudit(status.auditEvents),
      message: status.message,
      lines: status.lines,
      runtimeInfo: status.runtimeInfo,
      lc0Audit: status.lc0Audit,
    };
  } finally {
...
async function runArenaSmoke(args) {
  ...
  Object.entries({ seatA: 'centipawn:bt4-custom:1', seatB: 'centipawn:bt4-custom:1', games: 1, delayMs: 0, cacheEntries: 64 }).forEach(([k, v]) => {
  ...
  try {
    ...
    const status = await pollUntil(
      ...
      (value) => validateStrictCustomAudit('arena', value) && String(value.message ?? '').startsWith('Match done'),
      args.timeoutMs,
    );
    return {
      name: 'arena',
      status: 'LC0_CENTIPAWN_ARENA_STRICT_CUSTOM_WEBGPU_DONE',
      url: String(url),
      auditEvents: strictCustomAudit(status.auditEvents),
      message: status.message,
      pairing: status.pairing,
      matchScore: status.matchScore,
      runtimeBadge: status.runtimeBadge,
      runtimeInfo: status.runtimeInfo,
    };
  } finally {
    await closeSession(args, session);
...
async function main() {
  ...
  try {
    ...
    const summary = {
      schema: '0x88.centipawn-strict-custom-webgpu-smoke.v1',
