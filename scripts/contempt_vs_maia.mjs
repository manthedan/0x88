...
// Zipped with draw-scores to form per-arm (drawScore, scLimit, contemptElo)
// configs; a single value applies to every arm.
const SC_LIMITS = String(args.get('sc-limits') ?? '0').split(',').map(Number);
const CONTEMPT_ELOS = String(args.get('contempt-elos') ?? '0').split(',').map(Number);
...
function lc0Engine(drawScore, scLimit, contemptElo) {
  const searcher = new Lc0PuctSearcher(lc0Evaluator);
  return {
    name: `Lc0 v${VISITS} ds=${drawScore}${scLimit ? ` sc=${scLimit}` : ''}${contemptElo ? ` ce=${contemptElo}` : ''}`,
    searcher,
    async chooseMove(positions) {
      const result = await searcher.search({ positions }, { visits: VISITS, reuseTree: true, drawScore, searchContemptLimit: scLimit, contemptElo });
      return result.move ?? null;
    },
  };
}

const arms = DRAW_SCORES.map((drawScore, i) => ({
  drawScore,
  scLimit: SC_LIMITS[Math.min(i, SC_LIMITS.length - 1)],
  contemptElo: CONTEMPT_ELOS[Math.min(i, CONTEMPT_ELOS.length - 1)],
}));
console.log(`contempt A/B vs ${maia.name} · ${GAMES} games/arm · v${VISITS} · odds=${ODDS} · maxPlies=${MAX_PLIES} · arms=${arms.map((a) => `ds${a.drawScore}/sc${a.scLimit}/ce${a.contemptElo}`).join(' ')}`);
for (const { drawScore, scLimit, contemptElo } of arms) {
  const lc0 = lc0Engine(drawScore, scLimit, contemptElo);
  ...
  for (let game = 0; game < GAMES; game++) {
    ...
    console.log(`  ds=${drawScore}/sc=${scLimit}/ce=${contemptElo} game ${game + 1}/${GAMES} (lc0=${lc0IsWhite ? 'W' : 'B'}): ${out.result} ${out.reason} in ${out.plies} plies -> ${lc0Result}`);
  }
  const score = wins + draws / 2;
  console.log(`drawScore=${drawScore} scLimit=${scLimit} contemptElo=${contemptElo}: +${wins} =${draws} -${losses} · score ${score}/${GAMES} (${Math.round((score / GAMES) * 100)}%) · avg ${Math.round(plies / GAMES)} plies · ${[...reasons].map(([reason, count]) => `${reason}x${count}`).join(' ')}`);
}
