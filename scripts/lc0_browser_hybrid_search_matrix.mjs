...
async function runCell(args, combo, index, total) {
  ...
  process.stderr.write(`[matrix] ${index}/${total} repeat=${combo.repeat} kernel=${combo.encoderKernel} backend=${combo.headBackend} visits=${combo.visits} batch=${combo.batch} pipe=${combo.batchPipelineDepth}\n`);
      ...
      for (const headBackend of args.headBackends) {
        for (const visits of args.visits) {
          for (const batch of args.batches) {
            for (const batchPipelineDepth of args.batchPipelineDepths) combos.push({ repeat, encoderKernel, headBackend, visits, batch, batchPipelineDepth });
          }
        }
      }
    }
