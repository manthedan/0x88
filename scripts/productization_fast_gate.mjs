...
function commandPlan(args) {
  ...
  return [
    ...
    {
      name: 'runtime-productization-tests',
      command: 'node',
      args: [
        '--experimental-strip-types',
        '--test',
        'tests/engine_catalog.test.mjs',
        'tests/lc0_analysis_format.test.mjs',
        'tests/lc0_stable_backend_defaults.test.mjs',
      ],
    },
    {
      name: args.strictBrowserSmoke ? 'centipawn-strict-custom-webgpu-smoke' : 'centipawn-strict-custom-webgpu-smoke-dry-run',
      command: 'npm',
      args: ['run', 'lc0:centipawn-strict-custom-webgpu-smoke', '--', ...smokeArgs],
    },
  ];
}
...
async function main() {
  ...
  try {
    ...
    const artifact = {
      schema: '0x88.targeted-productization-smoke.v1',
      status: 'TARGETED_PRODUCTIZATION_SMOKE_DONE',
      startedAt,
      finishedAt: new Date().toISOString(),
      strictBrowserSmoke: args.strictBrowserSmoke,
      rows,
    };
    if (args.out) {
    ...
    const artifact = {
      schema: '0x88.targeted-productization-smoke.v1',
      status: 'TARGETED_PRODUCTIZATION_SMOKE_FAILED',
      startedAt,
      finishedAt: new Date().toISOString(),
      strictBrowserSmoke: args.strictBrowserSmoke,
      rows,
