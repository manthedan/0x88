...
- Added the smolgen isolated parity/profiling harness (`?smolgenBench=1`) as the next-roadmap foundation for generated schedule exploration. The harness runs smolgen compress → dense1 → LN1 → dense2 → LN2 → configurable project kernel against the CPU smolgen reference, reports final bias drift, and times each stage independently. Smoke command: `lc0-policy-only.html?smolgenBench=1&smolgenIters=10&smolgenWarmup=1&packVerify=0`; smoke result passed with max abs error `4.768e-6`, RMS `8.04e-7`, and stage averages roughly project `0.178 ms`, dense1 `0.101 ms`, compress `0.0545 ms`, dense2 `0.053 ms`, LN2 `0.0465 ms`, LN1 `0.039 ms`.
- Ran the isolated smolgen project tile search with `smolgenIters=50`, `smolgenWarmup=5`, `packVerify=0`, all passing final-bias parity at max abs error `4.768e-6`. Per-stage project averages were: hand `0.1806 ms`, tile16 `0.1152 ms`, tile32 `0.0759 ms`, tile64/current `0.0705 ms`, tile128 `0.0591 ms`, tile256 `0.0690 ms`. This is useful profiler evidence only: previous full-search 128/256 project variants lost to the recovered 64-column runtime lane, so no runtime promotion follows without fixed-suite confirmation.
- Followed up the isolated tile128 signal with a temporary full-search opt-in runtime variant using the 128-column smolgen project tile. It passed the 3-fixture drift guard (`/tmp/lc0_autoresearch/20260606_111743_51263_drift.json`) but regressed badly on a 16-position recovered fixed suite: tile128 `6.464 ms/eval` (`/tmp/lc0_autoresearch/20260606_111743_51263_rep1.json`) versus same-session-style current tile64 control `4.954 ms/eval` (`/tmp/lc0_autoresearch/20260606_111815_51606_rep1.json`). Reverted the temporary runtime variant; kept only the isolated harness support.
- Rechecked the existing isolated attention block score+softmax fusion harness before considering more full-search attention work. Baseline separated score+softmax attention block passed parity at max abs `1.25e-6` and `0.274 ms/iter` synced (`attentionBlockBench=1&attentionBlockIters=100&attentionBlockWarmup=5&packVerify=0`). The scalar fused score+softmax path also passed parity at max abs `1.25e-6` but was about 2x slower at `0.560 ms/iter` despite reducing dispatches (`fusedScoreSoftmax=1`), matching the prior no-promotion decision.
