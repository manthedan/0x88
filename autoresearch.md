...
2. Keep `mixed-tvm-ffn-smolgen-project` as the current best explicit encoder-kernel opt-in, now with the 64-column smolgen-project tile plus an 8-lane parallel-reduction smolgen-dense1 kernel. Earlier simple one-output-per-lane dense1, dense1 fusion, and row-split attempts remain discarded.
