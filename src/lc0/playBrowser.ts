...
const ENGINE_OPTIONS: PlayEngineOption[] = isV0DeployProfile()
  ? ALL_ENGINE_OPTIONS.filter((option) => option.family === 'maia3' || option.id === 'leela-queen-odds' || option.id === 'sf-lite' || option.id === 'lc0-small' || option.id === 'berserk' || option.id === 'viridithas' || option.id === 'plentychess')
  : ALL_ENGINE_OPTIONS;
