...
export interface BerserkOptions {
  ...
  hashMb?: number;
  /** Berserk is currently built as synchronous single-thread Emscripten; keep at 1 unless a pthread build is promoted. */
  threads?: number;
}
...
function abortError(message = 'Berserk search aborted'): Error {
...
export class BerserkEngine implements BrowserUciEngine {
  readonly name = 'berserk-emscripten';
  ...
  private options: BerserkOptions;
  private initialized = false;
  private queueTail: Promise<void> = Promise.resolve();
  private lastInfoLines: BerserkInfoLine[] = [];
  private readonly jsUrl: string;
  private readonly wasmUrl?: string;
  private readonly dataUrl?: string;

  constructor(options: BerserkOptions = {}, jsUrl: string = DEFAULT_BERSERK_EMSCRIPTEN_JS_URL, wasmUrl?: string, dataUrl?: string) {
  ...
  setOptions(next: BerserkOptions): void {
  ...
  private makeWorker(): Worker {
    ...
    worker.onerror = (event) => this.failAll(new Error(event.message || 'Berserk worker error'));
    this.worker = worker;
    return worker;
  }
  ...
  private waitForLine(predicate: (line: string) => boolean, label: string, startIndex: number, signal?: AbortSignal, timeoutMs = 20000): Promise<string> {
    ...
    return new Promise<string>((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(new Error(`Timed out waiting for ${label}; last Berserk lines: ${this.stdout.slice(-8).join(' | ')}`));
      }, timeoutMs);
      const onAbort = () => {
        clearTimeout(timer);
        this.lineWaiters = this.lineWaiters.filter((entry) => entry !== waiter);
  ...
  async analyze(fen: string, opts: BrowserUciAnalysisOptions = {}): Promise<BerserkInfoLine[]> {
    return this.runExclusive(async () => {
  ...
  private collectInfoLines(startIndex: number): BerserkInfoLine[] {
    const byPv = new Map<number, BerserkInfoLine>();
  ...
  dispose(): void {
    this.failAll(abortError('Berserk engine disposed'));
    this.stdout = [];
    this.stderr = [];
    this.lastInfoLines = [];
  }
