...
async function importEmscriptenGlue(resolvedJsUrl, trustedJsUrl) {
  ...
  if (!trustedJsUrl) throw new Error('Refusing to execute untrusted PlentyChess Emscripten glue: ' + resolvedJsUrl);
  const response = await fetch(resolvedJsUrl, { mode: 'cors', credentials: 'omit', cache: 'force-cache' });
  if (!response.ok) throw new Error('Failed to fetch PlentyChess Emscripten glue: ' + response.status + ' ' + resolvedJsUrl);
