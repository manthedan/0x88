...
export class StockfishEngine implements BrowserUciEngine {
  ...
  dispose(): void {
    this.disposed = true;
    // Reject active work before terminating the worker. Merely clearing these
    // callbacks leaves bestMove()/analyze()/waitReady() promises pending
    // forever and keeps the exclusive queue locked.
