<script lang="ts">
  ...
  type AnyGpuComputePass = JsonRecord & { setPipeline: (pipeline: unknown) => void; setBindGroup: (index: number, bindGroup: unknown) => void; dispatchWorkgroups: (count: number) => void; end: () => void };
  ...
  function readIframeBench(): { done: boolean; failed: boolean; frame: JsonRecord; bench: unknown } {
    ...
    if (benchText.startsWith('{')) {
      try { bench = JSON.parse(benchText); } catch {}
    }
    const message = text('message');
    const objectStatus = typeof bench === 'object' && bench !== null ? String((bench as JsonRecord).status ?? '') : '';
    const failed = objectStatus === 'BENCH_FAILED'
      || /^BENCH_FAILED\b/.test(benchText)
      || /^(?:Model load failed|Page failed to initialize):/i.test(message);
      ...
      try {
        ...
        if (observed.done || timedOut) {
          ...
          const report = {
            ...
            classificationHint: observed.failed ? { warning: 'LC0 benchmark failed; inspect lc0Bench.frame and result.' } : classifyResult(observed.bench, wgslSmoke),
          };
          ...
          status = observed.failed
            ...
            : observed.done ? 'Done. Copy the JSON below and send it back.' : 'Timed out. Copy the JSON below anyway.';
        }
      } catch (error) {
        if (pollTimer) clearInterval(pollTimer);
        finishedAt = nowIso();
        reportText = JSON.stringify({ ...partial, finishedAt, chromeGpuNotes, lc0Bench: { status: 'error', error: String(error), iframeUrl: iframeSrc } }, null, 2);
        runState = 'error';
  ...
  function classifyResult(bench: unknown, wgslSmoke: unknown): JsonRecord {
    ...
    return {
      ...
      warning: Number.isFinite(avgMs) && avgMs >= 500
