...
onMount(() => {
  ...
  let mounted = true;
  void import('../../lc0/policyOnlyBrowser')
    .then((module) => {
      if (!mounted) return;
...
<SiteHeader pageTitle="Single engine" />
<main id="main">
  <section class="panel" aria-label="Playable chess board">
  ...
  <aside class="panel app-sidebar" aria-label="LC0 debug panel">
...
<style>
  main{
    display:grid;
    grid-template-columns:minmax(320px,680px) minmax(360px,440px);
  ...
  :global(.controls){display:grid; grid-template-columns:repeat(6,1fr); gap:8px; margin-top:12px}
  :global(.controls button){padding:10px 8px}
  :global(.status-grid){display:grid; grid-template-columns:120px 1fr; gap:8px 12px; font-size:13px}
  :global(.status-grid dt){color:var(--muted)}
  :global(.status-grid dd){margin:0; overflow-wrap:anywhere}
  ...
  :global(.policy-list li){
    display:grid; grid-template-columns:24px 72px 1fr 64px;
    gap:8px; align-items:center; padding:6px 0;
    border-top:1px solid var(--rule); font-family:var(--mono); font-size:12px;
  }
  :global(.policy-list li.best b){color:var(--accent)}
  :global(meter){width:100%}
  :global(.fen){font-family:var(--mono); font-size:12px; line-height:1.45; overflow-wrap:anywhere}
  :global(.settings){margin-top:12px; display:grid; gap:8px}
  :global(.settings-row){display:flex; flex-wrap:wrap; gap:8px; align-items:end}
  :global(.settings .field){display:grid; gap:3px; flex:1 1 90px; min-width:80px; margin-top:0}
  :global(.settings .fen-field){flex:1 1 100%}
  :global(.settings label){font-size:11px; color:var(--muted)}
  ...
  :global(#gpuStatus.warn){color:#a5461b; font-weight:600}
  :global(.battle-list){list-style:none; margin:8px 0 0; padding:0; font-family:var(--mono); font-size:12px}
  :global(.battle-list li){padding:4px 0; border-top:1px solid var(--rule)}
