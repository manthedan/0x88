...
export function moveToPolicyIndex(move: Move): number | undefined {
  return POLICY_INDEX.get(moveToUci(move));
}
export function policyIndexToMove(index: number): Move {
  return moveFromUci(POLICY_MOVES[index]);
}
