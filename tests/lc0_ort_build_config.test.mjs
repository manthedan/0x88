...
test('ORT Relaxed SIMD build config pins matched fixed and relaxed artifacts', () => {
  ...
  assert.deepEqual(config.artifacts.fixed, ['ort-wasm-simd-threaded.asyncify.mjs', 'ort-wasm-simd-threaded.asyncify.wasm']);
  assert.deepEqual(config.artifacts.relaxed, ['ort-wasm-relaxedsimd-threaded.asyncify.mjs', 'ort-wasm-relaxedsimd-threaded.asyncify.wasm']);
