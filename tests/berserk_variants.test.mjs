...
test('Berserk variants pin Emscripten smoke and planned WASI metadata', () => {
  ...
  const expectedVariants = ['emscripten', 'emscripten-simd', ...(supportsWasmRelaxedSimd() ? ['emscripten-relaxed'] : []), 'default', 'simd'];
  assert.deepEqual(BERSERK_VARIANTS.map((variant) => variant.key), expectedVariants);
