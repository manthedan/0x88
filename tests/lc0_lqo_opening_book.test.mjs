...
test('LQO black opening book covers normal first moves and returns legal replies', () => {
  ...
  assert.equal(
    lqoBlackBookMove(afterE4, 1, history, () => 0),
    moves[0].uci,
  );
});
...
test('LQO black opening book falls back when a first move is uncovered', () => {
  ...
  assert.equal(
    lqoBlackBookMove(afterF3, 1, ['f2f3'], () => 0),
    null,
  );
});
...
test('LQO black opening book is limited to black early plies', () => {
  ...
  assert.equal(
    lqoBlackBookMove(start, 0, [], () => 0),
    null,
  );
  const afterE4 = playUci(start, 'e2e4');
  assert.equal(
    lqoBlackBookMove(afterE4, 6, ['e2e4'], () => 0),
    null,
  );
});
...
test('LQO white first move samples from model policy moves that are also in book', () => {
  ...
  assert.equal(
    lqoWhiteFirstPolicyBookMove(start, 0, [], legalPriors, () => 0, 1),
    null,
  );
  assert.equal(
    lqoWhiteFirstPolicyBookMove(start, 0, [], legalPriors, () => 0, 3),
    'g1f3',
  );
  assert.equal(
    lqoWhiteFirstPolicyBookMove(start, 0, [], legalPriors, () => 0.999, 3),
    'c2c4',
  );
  assert.equal(
    lqoWhiteFirstPolicyBookMove(playUci(start, 'g1f3'), 1, ['g1f3'], legalPriors, () => 0),
    null,
  );
});
...
test('LQO white opening book covers the second white move before search', () => {
  ...
  assert.equal(
    lqoWhiteBookMove(board, 2, history, () => 0),
    moves[0].uci,
  );
});
