...
use tiny_leela_core::{
    encode_tactical_moveformer_legal_inputs, move_to_action_id,
...
fn group_labels(groups: &[MoveFormerFeatureGroup]) -> Vec<&'static str> {
    groups
        .iter()
        .map(|g| match g {
            MoveFormerFeatureGroup::Base => "base",
            MoveFormerFeatureGroup::Attack => "attack",
            MoveFormerFeatureGroup::Delta => "delta",
            MoveFormerFeatureGroup::Ray => "ray",
            MoveFormerFeatureGroup::KingZone => "kingzone",
        })
        .collect()
}
...
fn main() {
    ...
    let out = arg(
        "--out",
        "artifacts/cache_moveformer_tactical_rust/shard_0000",
    );
    ...
    let groups_spec = arg("--feature-groups", "base,attack");
    if input.is_empty() {
        panic!("--input is required");
    }
    let groups = moveformer_feature_groups_from_spec(&groups_spec).expect("parse feature groups");
    let feature_names = moveformer_feature_names_for_groups(&groups);
    ...
    let tmp_path = out_path.with_file_name(format!(
        ...
        out_path
            .file_name()
            .and_then(|s| s.to_str())
            .unwrap_or("moveformer_tactical_cache"),
        std::process::id()
    ));
    if tmp_path.exists() {
    ...
    'outer: for path in &inputs {
        for line in input_lines(path) {
            ...
            let (moves, ids, features, mask) =
                encode_tactical_moveformer_legal_inputs(&board, max_legal_moves, &groups);
            if moves.len() > max_legal_moves {
                truncated += 1;
            }
            legal_total += moves.len();
            let mut slot = -1i16;
    ...
    let meta = json!({
        ...
        "feature_groups": group_labels(&groups),
        "move_feature_names": feature_names,
        ...
        "bad_fen_rows": bad_fen,
        "producer": { "language": "rust", "binary": "tiny-leela-rust-moveformer-tactical-cache" },
    });
    fs::write(
    ...
    if !manifest_out.is_empty() {
        let manifest = json!({
            ...
            "producer": { "language": "rust", "binary": "tiny-leela-rust-moveformer-tactical-cache", "contract_versions": ["cache_manifest_v1"] },
            "arrays": [
    ...
    println!("METRIC moveformer_tactical_cache_rows={written}");
    println!("METRIC moveformer_tactical_cache_num_features={num_features}");
    println!(
        "METRIC moveformer_tactical_cache_target_legal_rate={:.6}",
