...
fn result_for_turn(white_score: f32, turn: Color) -> [u8; 3] {
    if (white_score - 0.5).abs() < 1e-6 { return [0, 1, 0]; }
    let side_won = (white_score == 1.0 && turn == Color::White) || (white_score == 0.0 && turn == Color::Black);
    if side_won { [1, 0, 0] } else { [0, 0, 1] }
}
...
fn main() {
    ...
    eprintln!("[rust-selfplay visits={visits}] start games={games} max_plies={max_plies} out={out_path}");
    ...
    for game in 0..games {
        ...
        for ply in 0..max_plies {
            ...
            if white_score.is_some() { break; }
            let result = search_root(&board, &evaluator, SearchOptions { visits, cpuct: 1.5, temperature });
            if result.mv.is_none() || result.policy.is_empty() { white_score = Some(0.5); break; }
            ...
            for entry in &result.policy {
                r -= entry.probability;
                if r <= 0.0 { chosen = entry.mv; break; }
            }
        ...
        let (score, adjudicated) = if let Some(score) = white_score { (score, false) } else { adjudicated_white_score(&board, &evaluator, &adjudicate, adjudicate_threshold) };
        if adjudicated { adjudicated_games += 1; }
        if (score - 0.5).abs() > 1e-6 { decisive_games += 1; }
    ...
    if let Some(parent) = std::path::Path::new(&out_path).parent() { fs::create_dir_all(parent).expect("create output dir"); }
    let body = rows.iter().map(|row| serde_json::to_string(row).unwrap()).collect::<Vec<_>>().join("\n");
    fs::write(&out_path, if body.is_empty() { String::new() } else { format!("{body}\n") }).expect("write jsonl");
    ...
    println!("METRIC selfplay_decisive_rate={:.6}", decisive_games as f64 / completed_games.max(1) as f64);
    println!("METRIC selfplay_adjudicated_rate={:.6}", adjudicated_games as f64 / completed_games.max(1) as f64);
    println!("METRIC selfplay_policy_mass={:.6}", policy_mass as f64 / rows.len().max(1) as f64);
