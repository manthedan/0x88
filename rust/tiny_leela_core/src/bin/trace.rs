...
use tiny_leela_core::{legal_moves, make_move, move_to_action_id, move_to_uci, parse_fen, StudentEvaluator, PositionEvaluator, START_FEN};

struct Edge { mv: tiny_leela_core::Move, prior: f32, visits: u32, value_sum: f32 }
fn q(e: &Edge) -> f32 { if e.visits > 0 { -e.value_sum / e.visits as f32 } else { 0.0 } }
fn value(wdl: [f32; 3]) -> f32 { wdl[0] - wdl[2] }
...
fn main() {
    ...
    let sims: u32 = std::env::args().nth(3).and_then(|s| s.parse().ok()).unwrap_or(8);
    ...
    let raw: Vec<f32> = moves.iter().map(|&m| map.get(&move_to_action_id(m)).copied().unwrap_or(0.0).max(0.0)).collect();
    let total: f32 = raw.iter().sum();
    let fallback = if moves.is_empty() { 0.0 } else { 1.0 / moves.len() as f32 };
    let mut edges: Vec<Edge> = moves.into_iter().enumerate().map(|(i, mv)| Edge { mv, prior: if total > 0.0 { raw[i] / total } else { fallback }, visits: 0, value_sum: 0.0 }).collect();
    println!("root_value={:.9}", value(evaln.wdl));
    for (i, e) in edges.iter().enumerate().take(8) { println!("PRIOR {i} {} {:.9}", move_to_uci(e.mv), e.prior); }
    for sim in 0..sims {
