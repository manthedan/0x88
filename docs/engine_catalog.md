...
- **Open work:** keep extracting shared catalog metadata into UI config; keep experimental LC0 runtime lanes opt-in until repeated parity, leak, and cross-host latency gates pass. After the naive WebGPU int8 FFN lane regressed throughput despite passing drift, the next high-ROI GPU lane is smolgen f16 tiling/generation. The first opt-in keep is `mixed-tvm-ffn-smolgen-project`, which confirmed at `5.416 ms/eval` on the cleaned mixed-TVM JS-legal 32-position fixed suite with 9-fixture drift passing. Simple tiled smolgen-dense1, combined project+dense1, dense1+swish+LN1 fusion, row-split dense1 reduction, 16/32/128/256-column project tiles, manual project-loop unrolling, shared-input smolgen dense2/compress, project+TVM outproj/QKV, split-FFN TVM mixes, GPU/WASM legal priors, JS/WGSL input, and project+attention-score fusion attempts did not improve or failed drift, so future smolgen work needs a different generated/fused schedule with isolated parity first.
