...
| PlentyChess | none yet | Browser-smoke only | Patched single-thread Emscripten UCI worker; no arena/analysis selector yet | `Yoshie2000/PlentyChess` commit `58d8ba2505ae2b49f48dd410d214a457d15c12c6` + network `0134-2r24-s0.bin` | JS ~71 KB + WASM ~390 KB + data/processed NNUE ~63 MB; generated/local only | Node and browser lifecycle smoke pass; next gate is benchmark/footprint decision before any selector integration. |
...
- **Integration status:** browser-smoke only. Emscripten build, Node UCI smoke, reusable worker adapter, and browser lifecycle smoke pass, but it is not benchmarked or exposed in staged selectors yet.
