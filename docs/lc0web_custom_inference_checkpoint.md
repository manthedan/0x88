...
- Isolated WGSL policy/value-head probe: `?wgslHeadsProbe=1&ep=wasm&packVerify=0`
  ...
  - Representative errors after adding WGSL policy logits: policy dense max abs error about `9.83e-7`, main 64×64 policy-logit max abs error about `8.64e-7`, value embed max abs error about `3.28e-7`, and WGSL WDL max abs error about `5.96e-8`. Policy dense, policy logits, value embed, and WGSL WDL readbacks were all nonzero and nonuniform. The existing tiny ORT head path on the same input produced WDL `[0.2996, 0.3884, 0.3120]` with WDL-vs-reference max abs error about `2.98e-8`.
