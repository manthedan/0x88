...
- Isolated WGSL policy/value-head probe: `?wgslHeadsProbe=1&ep=wasm&packVerify=0`
  ...
  - The probe is intentionally not wired into the evaluator: it dispatches first policy dense+mish plus the full value-head path on a deterministic encoder-shaped input, reads those buffers back, and compares them to the CPU f32 head reference.
  - Representative errors after adding the full WGSL value head: policy dense max abs error about `9.83e-7`, value embed max abs error about `3.28e-7`, and WGSL WDL max abs error about `5.96e-8`. Policy dense, value embed, and WGSL WDL readbacks were all nonzero and nonuniform. The existing tiny ORT head path on the same input produced WDL `[0.2996, 0.3884, 0.3120]` with WDL-vs-reference max abs error about `2.98e-8`.
