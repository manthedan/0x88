...
- `?wgslHeadsProbe=1`: isolated experimental WGSL policy/value-head dense probe. It dispatches only the first policy dense+mish and value embed+mish kernels on a deterministic encoder-shaped input, reads those intermediate buffers back, compares them to the CPU f32 head reference, and runs the existing ORT tiny-head path for final mapped-policy/WDL context. This is not wired into the hybrid evaluator.
...
- Isolated WGSL policy/value-head probe: `?wgslHeadsProbe=1&ep=wasm&packVerify=0`
  ...
  - The probe is intentionally not wired into the evaluator: it dispatches only first policy dense+mish and value embed+mish kernels on a deterministic encoder-shaped input, reads those intermediate buffers back, and compares them to the CPU f32 head reference.
  - Representative intermediate errors: policy dense max abs error about `9.83e-7`, value embed max abs error about `3.28e-7`. Both readback buffers were nonzero and nonuniform. The existing tiny ORT head path on the same input produced WDL `[0.2996, 0.3884, 0.3120]` with WDL-vs-reference max abs error about `2.98e-8`.
