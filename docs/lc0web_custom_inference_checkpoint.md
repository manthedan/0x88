...
- SIMD WASM legal-prior prototype:
  ...
  - Validation: `node --experimental-strip-types --test tests/lc0_wasm_legal_priors.test.mjs` compares the WASM legal-prior output against JS `legalPolicyPriors` over all 10 representative FEN fixtures and all 6 explicit-history final FENs, including castling, en-passant, promotions, checks, and top-K output.
- Batch-fill audit:
