...
- CI-style browser smoke harness:
  ...
  - Local artifact: `/tmp/lc0_browser_ci_smoke_task4_reviewfix.json` from `npm run lc0:browser-ci-smoke -- --fixture-limit 1 --timeout 180000`. It passed all four smoke rows and ended with `LC0_BROWSER_CI_SMOKE_LEAK_CHECK_CLEAN`: stable ORT-heads and experimental WGSL-heads each returned `d2d4`; mapped-policy probe max abs error was `2.98e-8` with nonzero/nonuniform output; WGSL-heads-vs-ORT matched `1/1` best moves with mapped-policy max diff about `2.62e-6` and WDL max diff about `1.19e-7`.
