...
| Centipawn BT4 | `bt4_soap_rem_c19000_final` (`best-bt4`) | `/models/bt4_soap_rem_c19000_final.onnx`; metadata `/models/bt4_soap_rem_c19000_final.meta.json`; release source `/models/centipawn.manifest.json`; provenance `docs/model_provenance/bt4_soap_rem_c19000_final.md` | Tracked/deployable via Git LFS and R2 release channel | Production Centipawn ORT model. BT4/SquareFormer + ThreatGraph TG1 trained from LC0 public data with Stockfish labels; not ChessBench, TCEC, or generic human-game corpus. |
...
| Stormphrax | `Ciekce/Stormphrax` `v8.0.0`, commit `582965517ed2032d41a6b4cd6c2e66b1b934e2ad`; network `undertown.nnue` | `/stormphrax/stormphrax-emscripten.js`, `/stormphrax/stormphrax-emscripten.wasm`, `/stormphrax/stormphrax-emscripten.data`; manifest `/stormphrax/stormphrax-emscripten-single-thread.manifest.json` | Tracked/deployable, experimental | Single-thread WebAssembly SIMD Emscripten worker. Matching GPL source archive is committed. |
| Reckless | `codedeliveryservice/Reckless` commit `0010617448bd` + local browser/WASI patches; full v60, lite candidate v53 | `/reckless/reckless-relaxed-simd128.wasm`, `/reckless/reckless-simd128.wasm`, `/reckless/reckless.wasm`, browser-API variants, and optional NNUE sidecars | Ignored/local generated artifacts; README/NOTICE tracked | The default ladder is relaxed SIMD > fixed SIMD > scalar, with asset fallback at each step. Production must use canonical `npm run reckless:build-release`, include all three default-ladder WASM assets (including relaxed SIMD), generate the release manifest, and publish matching corresponding-source archives. |
...
- Engine-specific release manifests with artifact hashes and source-archive
  ...
  - `/stormphrax/stormphrax-emscripten-single-thread.manifest.json`
