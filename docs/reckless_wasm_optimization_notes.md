...
- **Smaller/lazier NNUE asset path**: full Reckless artifacts are ~99.5% data payload; see [`reckless_nnue_asset_size_plan.md`](./reckless_nnue_asset_size_plan.md). An initial browser API SIMD external-NNUE build now emits a ~1.26 MB WASM plus a separate ~63.27 MB `.nnue` payload and passes a depth-4 browser loading smoke. It remains experimental until deeper parity/performance validation; embedded SIMD WASI/UCI stays the default.
