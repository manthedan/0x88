...
- `scripts/validate_artifact_cdn_headers.mjs` validates the exact hosted immutable release against the supplied local manifest, then validates that release's physical identity/Brotli URLs, range behavior, target-aware cache policy, CORS/CORP, timing, no-cookie, cache-status, and integrity metadata. `--url` remains a separate logical-alias canary for the current channel. Full identity and decoded Brotli hashes require the explicit `--verify-bodies` integrity mode.
