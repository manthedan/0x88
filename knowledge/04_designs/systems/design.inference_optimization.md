...
Current LC0 browser-lane recommendation, 2026-06-05: productize only as explicit opt-ins the recovered-state `hybrid-wgsl-heads` configurations with WASM input, JS legal priors, batch size 4, and `batchPipelineDepth=1`. Use **LC0 WebGPU Research b4** as the script/docs preset name for the current best opt-in runtime/search bundle, not as a UI default or benchmark protocol. The preset means `hybrid-wgsl-heads`, WASM input, `mixed-tvm-ffn-smolgen-project`, JS legal priors, Leaf Batch size 4, and Speculative Search Pipeline depth 1; fixture suite, repeats, warmup, drift thresholds, and browser cleanup policy belong to benchmark protocols instead. The baseline opt-in is `mixed-tvm-ffn`; the stronger research opt-in is now `mixed-tvm-ffn-smolgen-project`, a 64-column tiled-f16 smolgen-project plus 8-lane smolgen-dense1 variant with 2-step row-loop unroll confirmed at `4.863 ms/eval` on the 32-position suite with 9-fixture drift passing. Deprioritize GPU/WASM legal priors, JS/WGSL input, TVM QKV/outproj combinations, split-FFN TVM dense1-only/dense2-only mixes, pipe2/depth>1 fixed-suite runs, batch-size sweeps, and readback micro-toggles unless new full-search attribution changes the bottleneck. A simple WebGPU int8 FFN lane passed drift but regressed throughput; simple tiled smolgen-dense1/project+dense1, dense1+swish+LN1 fusion, row-split dense1 reduction, discarded dense1 shape/unroll/fma variants including the 3-step row-loop unroll that failed drift, paired-output/dense2-unroll variants, compress/head scalar unrolls/workgroup-size tweaks/packed-f16 buffers/linear-specialized shaders/value dense2+softmax fusion, hand QKV/output-projection larger K tiles, generated TVM `pick_lane` select rewrite, attention-score channel unroll, parallel score+softmax fusion, attention-value loop unrolls/workgroup reshapes, transposed-weight smolgen compress/dense1/dense2, paired-output smolgen project, smolgen dense2/compress workgroup-shape/reduction tweaks, mapped-policy 128-wide groups, and 16/32/128/256-column project tile follow-ups, manual project-loop unrolling, project weight-index strength reduction, project final-barrier elision, shared-input smolgen dense2/compress, and LN2 swish caching were slower than project-only; and project+attention-score fusion attempts either failed drift or passed parity but regressed throughput. The next high-ROI GPU axis must therefore be a materially different generated/fused smolgen schedule with isolated parity first and the same fixed-suite speed/drift gates. Treat CPU/WASM int8 as a separate fallback lane, not the current WebGPU priority. Treat hybrid-runtime Runtime Footprint, caching, and compression as a separate Inference Optimization branch: instrument model-pack bytes, GPU buffer footprint, browser heap where available, evaluator cache hit rate, activation/readback buffer reuse, and per-position cache effectiveness before attempting compression or cache-policy changes. Treat Runtime Footprint as a Pareto axis rather than a replacement for throughput: keep footprint changes only when they avoid material Fixed-suite Search Throughput regression, or when they deliberately create a documented low-footprint Opt-in Runtime Configuration; Drift Checks still apply, and artifacts should report footprint and throughput together whenever possible. For attribution, prefer explicit lc0web accounting over browser memory APIs: report Pack Footprint and Execution Footprint from known model artifact bytes, decoded tensor bytes, GPU buffer sizes, cache entries, and reusable activation/readback buffers, while treating browser heap/user-agent memory samples as opportunistic supporting evidence only. Start by extending the existing pack-load probe with a structured `packFootprint` object, then propagate compatible footprint objects into hybrid init/eval/search artifacts; add `executionFootprint` after GPU buffer accounting is explicit. The first `executionFootprint` pass should report lc0web-owned persistent GPU buffer categories, physical slot/layer counts, batch upload/readback capacities, and exclusions; use it to prioritize immutable smolgen weight sharing, encoder scratch reuse, cache-hit instrumentation, packaging compression, and post-upload JS pack release in that order. Pack Footprint should report both whole-pack and run-specific values: declared tensor bytes and total shard bytes from the manifest, plus loaded tensor bytes and loaded shard bytes for the tensors actually materialized by the run. Use `packFootprint` as the canonical artifact object with `declaredTensorBytes`, `loadedTensorBytes`, `totalShardBytes`, `loadedShardBytes`, `tensorCount`, `loadedTensorCount`, `shardCount`, `loadedShardCount`, and `dtypeHistogram`; keep existing flat pack-load fields only for backward compatibility during transition. Use `cacheFootprint` for evaluator-cache attribution with `entries`, `maxEntries`, `approxBytes`, `approxKeyBytes`, `approxEvaluationBytes`, and an explicit approximation note; report it separately from GPU-buffer `executionFootprint` because JS object/map overhead and backend resources are intentionally excluded.
