...
- If future smolgen dense1 work resumes, try a materially different generated schedule, not the reverted one-output-per-lane shared-input tile and not the reverted one-workgroup dense1+swish+LN1 fusion. The fusion reduced dispatches but lost enough matmul parallelism to miss current-best throughput and had larger prior drift.
