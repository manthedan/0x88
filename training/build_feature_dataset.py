...
for r in rows:
    ...
    y.append(tv); yv.append([float(v) for v in r['wdl']]); w.append(float(r.get('_weight',1.0))); kept.append(r.get('fen',''))
obj={'moves':moves,'X':X,'y_policy':y,'y_wdl':yv,'weights':w,'fens':kept,'feature_dim':len(X[0]) if X else 0}
