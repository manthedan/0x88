...
def ce_policy(X,y,wp,ids=None):
  loss=top1=top4=top8=0
  for n,(x,t) in enumerate(zip(X,y)):
    logits=[sum(x[j]*wp[j][k] for j in range(len(x))) for k in range(len(wp[0]))]
    m=max(logits); s=sum(math.exp(v-m) for v in logits)
    loss-=sum(t[k]*(logits[k]-m-math.log(s)) for k in range(len(t)))
    target=ids[n] if ids is not None and ids[n] >= 0 else max(range(len(t)), key=t.__getitem__)
  ...
  den=max(1,len(X)); return loss/den, top1/den, top4/den, top8/den
...
X=ds['X']; yp=ds['y_policy']; yi=ds.get('y_move_id',[-1]*len(X)); yv=ds['y_wdl']; weights=ds['weights']; moves=ds['moves']; F=ds['feature_dim']; M=len(moves)
...
if args.preload_device:
  x=Tensor([X[i] for i in train]).realize(); y=Tensor([yp[i] for i in train]).realize(); ids=Tensor([yi[i] for i in train]).realize(); v=Tensor([yv[i] for i in train]).realize(); rw=Tensor([weights[i] for i in train]).reshape(len(train),1).realize()
  ...
  for _ in range(args.epochs):
    ...
    for off in range(0,len(train),args.batch_size):
      ...
      x=Tensor([X[i] for i in idx]); y=Tensor([yp[i] for i in idx]); ids=Tensor([yi[i] for i in idx]); v=Tensor([yv[i] for i in idx]); rw=Tensor([weights[i] for i in idx]).reshape(len(idx),1)
      opt.zero_grad(); lp=x.matmul(wp).log_softmax(); lv=x.matmul(ww).log_softmax()
...
trI=train[:min(len(train),2000)]; trX=[X[i] for i in trI]; trY=[yp[i] for i in trI]; trIds=[yi[i] for i in trI]; trV=[yv[i] for i in trI]
dX=[X[i] for i in dev]; dY=[yp[i] for i in dev]; dIds=[yi[i] for i in dev]; dV=[yv[i] for i in dev]
dp,top1,top4,top8=ce_policy(dX,dY,wp_l,dIds); dv=ce_wdl(dX,dV,ww_l); tp,_,_,_=ce_policy(trX,trY,wp_l,trIds); tv=ce_wdl(trX,trV,ww_l); score=100/(1+dp+dv)
...
print(f'METRIC distill_student_score={score:.6f}'); print(f'METRIC train_policy_ce={tp:.6f}'); print(f'METRIC train_wdl_ce={tv:.6f}'); print(f'METRIC dev_policy_ce={dp:.6f}'); print(f'METRIC dev_wdl_ce={dv:.6f}'); print(f'METRIC dev_policy_top1={top1:.6f}'); print(f'METRIC dev_policy_top4={top4:.6f}'); print(f'METRIC dev_policy_top8={top8:.6f}'); print(f'METRIC feature_dataset_rows={len(X)}'); print(f'METRIC batch_size={args.batch_size}'); print(f'METRIC preload_device={1 if args.preload_device else 0}'); print(f'METRIC sparse_policy={1 if can_sparse else 0}'); print(f'METRIC model_json_bytes={out.stat().st_size}')
