...
p=argparse.ArgumentParser(); p.add_argument('--dataset',required=True); p.add_argument('--out',required=True); p.add_argument('--epochs',type=int,default=40); p.add_argument('--lr',type=float,default=0.05); p.add_argument('--batch-size',type=int,default=512); p.add_argument('--holdout-mod',type=int,default=2); args=p.parse_args()
...
print(f'METRIC distill_student_score={score:.6f}'); print(f'METRIC train_policy_ce={tp:.6f}'); print(f'METRIC train_wdl_ce={tv:.6f}'); print(f'METRIC dev_policy_ce={dp:.6f}'); print(f'METRIC dev_wdl_ce={dv:.6f}'); print(f'METRIC dev_policy_top1={top:.6f}'); print(f'METRIC feature_dataset_rows={len(X)}'); print(f'METRIC batch_size={args.batch_size}'); print(f'METRIC model_json_bytes={out.stat().st_size}')
