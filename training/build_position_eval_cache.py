...
def valid_row(line, history):
    ...
    try:
        ...
        return None
    if r.get("schema") != SCHEMA_POSITION_EVAL or not r.get("fen"):
        return None
...
def build(inputs, out, max_rows, history):
    ...
    if n <= 0:
        raise SystemExit("no valid position-eval rows")
    ...
    q = np.memmap(out / "q.float32", np.float32, "w+", shape=(n,))
    weight = np.memmap(out / "quality_weight.float32", np.float32, "w+", shape=(n,))
    i = 0
    for p in inputs:
        for line in open_lines(p):
            if i >= n:
            ...
            i += 1
            if i % 100000 == 0:
                print(f"METRIC position_eval_cache_rows_written={i}", flush=True)
    for a in (tokens, policy, wdl, q, weight):
        a.flush()
    meta = {
        "format": "compact_position_eval_cache_v1",
        "rows": n,
        "token_features": F,
        ...
        "files": {
            ...
            "wdl": "wdl.float32",
            "q": "q.float32",
            "quality_weight": "quality_weight.float32",
        },
    }
...
def main():
    ap = argparse.ArgumentParser(
        description="Build tensorized compact cache for teacher.position_eval.v1 overlays."
    )
