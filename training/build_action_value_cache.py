...
def grouped_positions(paths: list[str], max_candidates: int, history: int):
    cur_key = None; cur_fen = None; cur_hist = None; cands: list[tuple[int, float, float, int, int]] = []
    source_order = 0
    def flush():
        ...
        ordered = sorted(cands, key=lambda x: ((x[3] if x[3] > 0 else 10**9), x[4], -x[1]))[:max_candidates]
        try:
    ...
    for r in iter_rows(paths):
        ...
        if cur_key is not None and key != cur_key:
            ...
            cands = []
        if key != cur_key:
            cur_key = key; cur_fen = r.get('fen'); cur_hist = r.get('history_fens') or []; source_order = 0
        source_order += 1
...
def count_positions(paths: list[str], max_positions: int, max_candidates: int, history: int) -> int:
    n = 0
...
def build(paths: list[str], out_dir: str | Path, max_positions: int, max_candidates: int, history: int) -> None:
    out = Path(out_dir); out.mkdir(parents=True, exist_ok=True)
    t0 = time.time(); F = history + 9
    ...
    moves = np.memmap(out / 'candidate_moves.int64', np.int64, 'w+', shape=(n, max_candidates))
    values = np.memmap(out / 'candidate_values.float32', np.float32, 'w+', shape=(n, max_candidates))
    regrets = np.memmap(out / 'candidate_regrets.float32', np.float32, 'w+', shape=(n, max_candidates))
    mask = np.memmap(out / 'candidate_mask.float32', np.float32, 'w+', shape=(n, max_candidates))
    moves[:] = 0; values[:] = 0.0; regrets[:] = 0.0; mask[:] = 0.0
    i = 0; candidate_rows = 0
    for tok, cands in grouped_positions(paths, max_candidates, history):
        ...
        for j, (mv, val, reg, _rank, _ord) in enumerate(cands[:max_candidates]):
            moves[i, j] = mv; values[i, j] = val; regrets[i, j] = reg; mask[i, j] = 1.0; candidate_rows += 1
        i += 1
        if i % 100000 == 0:
...
def main() -> int:
    ap = argparse.ArgumentParser(description='Build tensorized memmap cache for teacher.action_value.v1 overlays.')
    ...
    args = ap.parse_args()
    build(args.input, args.out, args.max_positions, args.max_candidates, args.history_plies)
    return 0
