...
FILES='abcdefgh'; PIECES='.PNBRQKpnbrqk'; PROMOS={'n':0,'b':1,'r':2,'q':3}
...
def valid_row(line, history):
    ...
    try: y=move_class(mv); x=encode(r['fen'],r.get('history_fens') or [],history)
    except Exception: return None
    wdl=np.asarray(r.get('wdl',[.25,.5,.25]),dtype=np.float32)
    return x,y,wdl
    ...
    for path in inputs:
        ...
        try:
            for line in f:
                ...
                n += 1
        finally: close_opener(f)
    return n,bad

def build(inputs,out,max_rows,history):
    out=Path(out); out.mkdir(parents=True,exist_ok=True); F=history+9
    n,bad=count_rows(inputs,max_rows,history)
    tokens=np.memmap(out/'tokens.uint8',np.uint8,'w+',shape=(n,64,F)); policy=np.memmap(out/'policy.int64',np.int64,'w+',shape=(n,)); wdl=np.memmap(out/'wdl.float32',np.float32,'w+',shape=(n,3))
    i=0
    for path in inputs:
        f=opener(path)
        try:
            for line in f:
                ...
                tokens[i]=rr[0]; policy[i]=rr[1]; wdl[i]=rr[2]; i+=1
                if i%100000==0: print(f'METRIC square_cache_rows_written={i}',flush=True)
        finally: close_opener(f)
    for a in (tokens,policy,wdl): a.flush()
    meta={'rows':n,'token_features':F,'history_plies':history,'policy_size':4096+4096*4,'format':'compact_square_tokens_v1','bad_or_skipped_rows':bad}
    (out/'meta.json').write_text(json.dumps(meta,separators=(',',':')))
    print(f'METRIC square_cache_rows={n}'); print(f'METRIC square_cache_token_features={F}'); print(f'METRIC square_cache_bad_rows={bad}')

def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--input',nargs='+',required=True); ap.add_argument('--out',required=True); ap.add_argument('--max-rows',type=int,default=0); ap.add_argument('--history-plies',type=int,default=2)
