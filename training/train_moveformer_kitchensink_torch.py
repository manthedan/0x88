...
def main():
    ...
    ap.add_argument('--batch-size', type=int, default=512); ap.add_argument('--av-batch-size', type=int, default=512); ap.add_argument('--max-candidates', type=int, default=48)
    ...
    sidecar_feature_names = list(sc['meta'].get('move_feature_names') or MOVE_FEATURE_NAMES)
    ...
    class Net(nn.Module):
        def __init__(self):
            ...
            enc=nn.TransformerEncoderLayer(d_model=d, nhead=args.heads, dim_feedforward=args.ff_dim, dropout=0.05, batch_first=True, activation='gelu', norm_first=True)
        ...
        def forward(self, planes, action_ids, move_features, legal_mask):
            ...
            # Use fp16-safe sentinels under autocast; -1e9 overflows half.
            pol=self.policy(tok).squeeze(-1).masked_fill(key_padding, -1e4)
            av=torch.tanh(self.av(tok).squeeze(-1)).masked_fill(key_padding, 0.0)
            rank=self.rank(tok).squeeze(-1).masked_fill(key_padding, -1e4)
            regret=F.softplus(self.regret(tok).squeeze(-1)).masked_fill(key_padding, 0.0)
    ...
    if args.resume:
        ...
        print(f'[moveformer-ks] resumed=1 missing={len(miss.missing_keys)} unexpected={len(miss.unexpected_keys)}', flush=True)
    else:
        print('[moveformer-ks] resumed=0', flush=True)
    ...
    def av_batch():
        ...
        if mf.shape[2] < MF:
            mf = np.pad(mf, ((0,0),(0,0),(0, MF - mf.shape[2])), mode='constant')
        elif mf.shape[2] > MF:
            mf = mf[:, :, :MF]
    ...
    if args.onnx_out:
        ...
        for kk in legal_ks:
            ...
            exported.append(str(out_path)); print(f'METRIC moveformer_onnx_export_k{kk}=1', flush=True)
