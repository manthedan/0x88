...
def main():
    ap=argparse.ArgumentParser(description='CNN-SquareFormer hybrid: residual CNN trunk + 64 square-token transformer + policy/WDL and optional candidate AV/rank/regret heads.')
    ap.add_argument('--manifest', required=True); ap.add_argument('--dev-cache', required=True)
    ap.add_argument('--av-cache', nargs='*', default=[], help='Optional ChessBench compact AV cache dirs or collection manifest. If omitted or --av-positions=0, trains supervised policy/WDL only.')
    ...
    if args.export_av_head and not use_av:
        print('[squareformer-hybrid] warning: --export-av-head requested without AV training; AV head will be randomly initialized/weak', flush=True)
    print(f'[squareformer-hybrid] policy_cache_shards={len(train)} policy_rows={sum(sizes)} input_planes={C}', flush=True)
    print(f'[squareformer-hybrid] av_cache_count={len(av_caches)} av_cache_rows={sum(c["rows"] for c in av_caches) if av_caches else 0} use_av={int(use_av)}', flush=True)
    print(f'[squareformer-hybrid] block_sampling policy_prefetch_rows={args.policy_prefetch_rows} av_prefetch_rows={args.av_prefetch_rows}', flush=True)
    ...
    class Net(nn.Module):
        def __init__(self):
            super().__init__(); ch=args.channels; d=args.square_dim
            self.stem=nn.Conv2d(C,ch,3,padding=1)
            self.blocks=nn.Sequential(*[Block(ch) for _ in range(args.blocks)])
            self.to_token=nn.Sequential(nn.Linear(ch,d), nn.LayerNorm(d)) if d != ch else nn.LayerNorm(d)
            self.pos=nn.Parameter(torch.randn(1,64,d)*0.02)
            enc=nn.TransformerEncoderLayer(d_model=d, nhead=args.square_heads, dim_feedforward=args.square_ff_dim, dropout=args.square_dropout, batch_first=True, activation='gelu', norm_first=True)
            ...
            self.rank=nn.Sequential(nn.Linear(d*4,d),nn.GELU(),nn.Linear(d,1)) if args.separate_aux_heads else None
            self.regret=nn.Sequential(nn.Linear(d*4,d),nn.GELU(),nn.Linear(d,1)) if args.separate_aux_heads else None
            idx=[]
            for sq in range(64):
        ...
        def conv_features(self,z): return self.blocks(F.relu(self.stem(z)))
        ...
        def forward(self,z):
            h=self.conv_features(z); tok=self.square_tokens(h); pooled=tok.mean(1); pf=torch.cat([tok.flatten(1),pooled],1) if args.policy_head=='hybrid' else tok.flatten(1); return self.policy(pf), self.wdl(pooled), tok
        def candidate_features(self,tok,mv):
            B,K=mv.shape; m=mv.to(device=tok.device,dtype=torch.long); ordinary=m < 4096; q=torch.div((m-4096).clamp_min(0),4,rounding_mode='floor')
            ...
            frp=self.sq_to_plane_idx[fr.clamp(0,63)]; top=self.sq_to_plane_idx[to.clamp(0,63)]; D=tok.shape[-1]
            hf=torch.gather(tok,1,frp[...,None].expand(-1,-1,D)); ht=torch.gather(tok,1,top[...,None].expand(-1,-1,D)); pooled=tok.mean(1)[:,None,:].expand(-1,K,-1); pe=self.promo_emb(pr.clamp(0,4)); return torch.cat([pooled,hf,ht,pe],-1)
        def av_scores(self,tok,mv): return self.av(self.candidate_features(tok,mv)).squeeze(-1)
        def rank_scores(self,tok,mv): return (self.rank if self.rank is not None else self.av)(self.candidate_features(tok,mv)).squeeze(-1)
        def regret_scores(self,tok,mv): return F.softplus((self.regret if self.regret is not None else self.av)(self.candidate_features(tok,mv)).squeeze(-1))
    net=Net().to(device)
    if args.resume:
        ...
        print(f'[squareformer-hybrid] resumed=1 missing={len(miss.missing_keys)} unexpected={len(miss.unexpected_keys)}', flush=True)
    else: print('[squareformer-hybrid] resumed=0', flush=True)
    ...
    def refill_av_pool():
        if not av_caches: raise RuntimeError('av_batch requested but no --av-cache was provided')
    ...
    def save(path, ep, step, metrics=None):
        Path(path).parent.mkdir(parents=True,exist_ok=True); torch.save({'model':net.state_dict(),'opt':opt.state_dict(),'epoch':ep,'step':step,'metrics':metrics or {},'args':vars(args),'kind':'squareformer_hybrid_av_v1'}, path)
    def eval_all(label):
        ...
        m={'dev_policy_ce':ce/max(1,seen),'dev_wdl_ce':wc/max(1,seen),'dev_policy_top1':t1/max(1,seen),'dev_policy_top4':t4/max(1,seen),'dev_policy_top8':t8/max(1,seen)}
        if use_av:
            m.update({'dev_av_mse':av_mse/max(1,av_pos),'dev_av_top1':av_ok/max(1,av_seen)})
    ...
    for ep in range(1,args.epochs+1):
        ...
        rng.shuffle(sched); st=time.time(); sums={}; counts={}; net.train()
    ...
    meta={'kind':'tiny_board_squareformer_hybrid_onnx_student','architecture':'cnn_square_transformer','policy_map':'uci_queen_knight_promo_v1','moves':moves,'channels':args.channels,'blocks':args.blocks,'policy_head':args.policy_head,'se':bool(args.se),'history_plies':args.history_plies,'input_planes':C,'trained_with_aux_av':bool(use_av),'av_head_exported':export_av,'aux_heads_exported':(['action_values','rank_scores','regrets'] if export_aux else (['action_values'] if export_av else [])),'separate_aux_heads':bool(args.separate_aux_heads),'action_value_move_encoding':'chessbench_compact_20480','onnx':args.onnx_out,'squareformer_dim':args.square_dim,'squareformer_heads':args.square_heads,'squareformer_layers':args.square_layers,'squareformer_ff_dim':args.square_ff_dim,'squareformer_dropout':args.square_dropout}
