...
def in_check_grid(grid, side):
  ...
  return False
def planes(fen):
  parts=fen.split(); board=parts[0]; side=parts[1] if len(parts)>1 else 'w'; castling=parts[2] if len(parts)>2 else '-'; ep=parts[3] if len(parts)>3 else '-'; n=22 if args.state_planes else 14; x=[[[0.0]*8 for _ in range(8)] for _ in range(n)]; grid=[['.']*8 for _ in range(8)]; r=f=0
  for ch in board:
    if ch=='/': r+=1; f=0
    elif ch.isdigit(): f+=int(ch)
    else: x[PIECES.index(ch)][r][f]=1.0; grid[r][f]=ch; f+=1
  ...
  if args.state_planes:
    ...
    for rr in range(8):
      for ff in range(8): x[18][rr][ff]=1.0; x[19][rr][ff]=1.0 if side=='w' else 0.0; x[20][rr][ff]=stm_check; x[21][rr][ff]=opp_check
  else:
    for rr in range(8):
...
class Net:
  def __init__(self):
    C=args.channels; self.c1=Conv2d(22 if args.state_planes else 14,C,3,padding=1); self.c2=Conv2d(C,C,3,padding=1); self.c3=Conv2d(C,C,3,padding=1); self.p=Linear(C*64 if args.policy_head=='spatial' else C,len(moves)); self.v=Linear(C,3)
  def __call__(self,x):
