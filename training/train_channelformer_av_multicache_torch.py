...
def main():
    ap=argparse.ArgumentParser(description='ChannelFormer-CNN: residual trunk + channel-token transformer policy/WDL, optional candidate AV/ranking/regret head.')
    ...
    ap.add_argument('--channels', type=int, default=64); ap.add_argument('--blocks', type=int, default=6); ap.add_argument('--policy-head', choices=['channelformer'], default='channelformer'); ap.add_argument('--se', action='store_true')
    ap.add_argument('--cpatch', type=int, default=32, help='Number of channel/concept tokens after trunk projection')
    ...
    if args.export_av_head and not use_av:
        print('[channelformer] warning: --export-av-head requested without AV training; AV head will be randomly initialized/weak', flush=True)
    print(f'[channelformer] policy_cache_shards={len(train)} policy_rows={sum(sizes)} input_planes={C}', flush=True)
    print(f'[channelformer] av_cache_count={len(av_caches)} av_cache_rows={sum(c["rows"] for c in av_caches) if av_caches else 0} use_av={int(use_av)}', flush=True)
    print(f'[channelformer] block_sampling policy_prefetch_rows={args.policy_prefetch_rows} av_prefetch_rows={args.av_prefetch_rows}', flush=True)
    ...
    class Net(nn.Module):
        def __init__(self):
            super().__init__(); ch=args.channels; cp=args.cpatch; d=args.channelformer_dim
            self.stem=nn.Conv2d(C,ch,3,padding=1)
            self.blocks=nn.Sequential(*[Block(ch) for _ in range(args.blocks)])
            self.channel_reduce=nn.Sequential(nn.Conv2d(ch,cp,1,bias=False), nn.BatchNorm2d(cp), nn.ReLU())
            self.to_token=nn.Sequential(nn.Linear(64,d), nn.LayerNorm(d))
            self.pos=nn.Parameter(torch.randn(1,cp,d)*0.02)
            enc=nn.TransformerEncoderLayer(d_model=d, nhead=args.channelformer_heads, dim_feedforward=args.channelformer_ff_dim, dropout=args.channelformer_dropout, batch_first=True, activation='gelu', norm_first=True)
            self.encoder=nn.TransformerEncoder(enc, num_layers=args.channelformer_layers)
            ...
            self.rank=nn.Sequential(nn.Linear(ch*4,ch),nn.GELU(),nn.Linear(ch,1)) if args.separate_aux_heads else None
            self.regret=nn.Sequential(nn.Linear(ch*4,ch),nn.GELU(),nn.Linear(ch,1)) if args.separate_aux_heads else None
            idx=[]
            for sq in range(64):
        ...
        def forward(self,z):
            h=self.features(z); summary=self.channel_summary(h); return self.policy(summary), self.wdl(summary), h
    ...
    if args.resume:
        ...
        print(f'[channelformer] resumed=1 missing={len(miss.missing_keys)} unexpected={len(miss.unexpected_keys)}', flush=True)
    else: print('[channelformer] resumed=0', flush=True)
    ...
    def save(path, ep, step, metrics=None):
        Path(path).parent.mkdir(parents=True,exist_ok=True); torch.save({'model':net.state_dict(),'opt':opt.state_dict(),'epoch':ep,'step':step,'metrics':metrics or {},'args':vars(args),'kind':'cnn_channelformer_av_v1'}, path)
    def eval_all(label):
    ...
    meta={'kind':'tiny_board_channelformer_onnx_student','architecture':'cnn_channel_transformer','policy_map':'uci_queen_knight_promo_v1','moves':moves,'channels':args.channels,'blocks':args.blocks,'policy_head':args.policy_head,'se':bool(args.se),'history_plies':args.history_plies,'input_planes':C,'trained_with_aux_av':bool(use_av),'av_head_exported':export_av,'aux_heads_exported':(['action_values','rank_scores','regrets'] if export_aux else (['action_values'] if export_av else [])),'separate_aux_heads':bool(args.separate_aux_heads),'action_value_move_encoding':'chessbench_compact_20480','onnx':args.onnx_out,'channelformer_cpatch':args.cpatch,'channelformer_dim':args.channelformer_dim,'channelformer_heads':args.channelformer_heads,'channelformer_layers':args.channelformer_layers,'channelformer_ff_dim':args.channelformer_ff_dim,'channelformer_dropout':args.channelformer_dropout}
