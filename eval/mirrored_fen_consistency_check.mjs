...
function mirrorBoard(b) {
  ...
  for (let i = 0; i < 64; i++) {
    const p = b.squares[i];
    if (p) squares[mirSq(i)] = `${p[0] === 'w' ? 'b' : 'w'}${p[1]}`;
  }
  return {
    squares,
    turn: b.turn === 'w' ? 'b' : 'w',
    castling: mirCastle(b.castling),
    epSquare: b.epSquare == null ? null : mirSq(b.epSquare),
    halfmove: b.halfmove,
    fullmove: b.fullmove,
  };
}
function mirrorUci(uci) {
  const m = moveFromUci(uci);
  return moveToUci({ from: mirSq(m.from), to: mirSq(m.to), promotion: m.promotion });
}
function symmetricPrior(uci) {
  const mu = mirrorUci(uci);
  const key = uci < mu ? uci : mu;
  let h = 0;
...
for (const fen of FENS) {
  ...
  for (const u of moves) {
    const p = symmetricPrior(u) / total,
      mp = symmetricPrior(mirrorUci(u)) / mtotal;
