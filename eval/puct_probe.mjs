...
const model=arg('--model'); const meta=arg('--meta'); const fen=arg('--fen','rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1'); const visitsList=arg('--visits','1,2,4,8,16,32').split(',').map(Number); const cpuct=Number(arg('--cpuct','1.5')); const topN=Number(arg('--top','12'));
...
for (const v of visitsList) {
  const r=await searchRoot(board,evaluator,{visits:v,cpuct,temperature:0});
