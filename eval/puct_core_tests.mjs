...
function policyFor(board, entries) {
  const m = new Map();
  for (const [uci, p] of Object.entries(entries)) {
    const mv = legalByUci(board, uci);
    if (mv) m.set(moveToActionId(mv), p);
  }
  return m;
}
function actionValuesFor(board, entries) {
  const m = new Map();
  for (const [uci, v] of Object.entries(entries)) {
    const mv = legalByUci(board, uci);
    if (mv) m.set(moveToActionId(mv), v);
  }
  return m;
}
...
function batchEvaluator(fn) {
  ...
  return {
    ...
    async evaluateBatch(boards, opts = []) {
      calls.batch++;
      calls.batchBoards += boards.length;
      return boards.map((b, i) => fn(b, opts[i] ?? {}));
    },
  };
}
function sequenceEvaluator(fn) {
  ...
  return {
    ...
    async evaluateBatchSequence(batches) {
      calls.sequence++;
      calls.sequenceBatches += batches.length;
      calls.sequenceBoards += batches.reduce((sum, b) => sum + b.boards.length, 0);
      return batches.map((batch) => batch.boards.map((b, i) => fn(b, batch.contexts?.[i] ?? {})));
    },
  };
}
...
async function testAllZeroPolicyUniformFallback() {
  ...
  assert.ok(
    priors.every((p) => Math.abs(p - 1 / 20) < 1e-12),
...
async function testGumbelRootPolicyIsBoundedToCandidateSet() {
  const ev = evaluator((board) => ({ policy: policyFor(board, { g1f3: 0.4, e2e4: 0.3, d2d4: 0.2, b1c3: 0.1 }), wdl: [0.5, 0, 0.5] }));
...
async function testLowTemperatureRootPolicyStaysFinite() {
  ...
  const edges = ['g1f3', 'e2e4'].map((uci, i) => ({
    move: legalByUci(board, uci),
    prior: i === 0 ? 0.6 : 0.4,
    child: null,
    visits: i === 0 ? 10000 : 9000,
    valueSum: 0,
    virtualVisits: 0,
    side: 'w',
  }));
  const policy = classicPuctPolicy.rootPolicy(edges, searchContext({ temperature: 0.01 }), {
    board,
    historyFens: [],
    expanded: true,
    terminalValue: null,
  ...
  assert.ok(
    policy.every((entry) => Number.isFinite(entry.probability)),
    `low-temperature probabilities should be finite: ${policy.map((e) => e.probability).join(',')}`,
  );
...
const tests = [
  ...
  testBatchedEvaluatorPath,
  testBatchedDuplicateLeafDoesNotBackupZero,
  testActionValuePolicyGuidesSelection,
  testGumbelRootPolicyIsBoundedToCandidateSet,
  testLowTemperatureRootPolicyStaysFinite,
  testPipelinedBackupCollisionModeDoesNotCrash,
];
